...
public class OmronPlc {
    ...
    public int rawRead(String area, String addr, int lengthOrIndex, byte[] resultBuffer,int readType) {        
        ...
        if (readType == 0) {
            frame[28] = this.getAreaCodeWord(area);
            frame[31] = 0x00;
            frame[32] = (byte) ((lengthOrIndex >> 8) & 0xFF);
            frame[33] = (byte) (lengthOrIndex & 0xFF);
        } else {
            frame[28] = this.getAreaCodeBit(area);
            frame[31] = (byte)lengthOrIndex;
            frame[32] = 0x00;
            frame[33] = 0x01;
        }
    ...
    public float readFloat(String area,String addr,int length){
        ...
        return bytes2Float(floatBuffer);
    }
    ...
    public int readInt(String area,String addr,int length){
        ...
        int result = bytes2Int(intBuffer); 
        return result;
    }

    public static int bytes2Int(byte[] intBuffer){
    ...
    public static float bytes2Float(byte[] buffer){
        int intValue = bytes2Int(buffer);
        float result = Float.intBitsToFloat(intValue);

        return result;
    }

    public byte[] verifyResultBuffer(byte[] rawBuffer,int length){
        boolean isSuccess = true;
        if (rawBuffer[11] == 3) {
            isSuccess = checkResultHead(rawBuffer[15]);
        }
        if (isSuccess && rawBuffer[27] == 1 && checkResultEnd(rawBuffer[28], rawBuffer[29])) {
            byte[] dataBuffer = new byte[length];
            System.arraycopy(rawBuffer, 30, dataBuffer, 0, dataBuffer.length);
            return dataBuffer;
        }
        return null;
    }
    ...
    private byte[] doRead(byte[] command, int length) {
        ...
        return this.verifyResultBuffer(resultBuffer, length);
    }
